 ...
 * @public
 */
export const STD_VIDEORES: Record<string, VideoResolution> = {
  "1080": {
    width: 1920,
    height: 1080,
  },
...
export class MediaDevicesHelper {
  ...
  static async probeAvailableVideoResolutions(deviceId: string): Promise<VideoResolution[]> {
    ...
    for (const res of Object.values(STD_VIDEORES)) {
      // entries are sorted in ascending order of keys
      ...
      await navigator.mediaDevices
        ...
        .catch((e) => {
          if (
            ...
            ["height", "width", "deviceId"].includes(e.constraint)
          ) {
        ...
        .then(
          ...
          (e) => {
            ...
            trace(`${T} probeAvailableVideoResolutions error`, {
              error: e.name,
              message: e.message,
              deviceId,
              width: res.width,
...
function filterDevicesList(
  ...
  const items = devicesList
    .filter((devInfo) => devInfo.kind === kind)
    .map((devInfo) => ({
